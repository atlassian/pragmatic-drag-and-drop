...
async function setup({ page }: { page: Page }) {
  ...
  const dropTargetInParent = page.locator(
    '[data-testid="drop-target-in-parent"]',
  );
  ...
  const dropTargetInIframe = iframe.locator(
    '[data-testid="drop-target-in-iframe"]',
  );
  const draggableInIframe = iframe.locator(
    '[data-testid="draggable-in-iframe"]',
  );
...
test.describe('iframes', () => {
  test('parent → iframe', async ({ page }) => {
    ...
    const { dropTargetInParent, dropTargetInIframe, draggableInParent } =
      await setup({ page });

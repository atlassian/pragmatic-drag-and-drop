...
it('should not update input published to users based on the "dragleave" event', () => {
  ...
  const cleanup = combine(
    ...
    draggable({
      ...
      onDrag: ({ location }) =>
        ordered.push(getEntry('draggable:drag', location.current.input)),
      onDrop: ({ location }) =>
        ordered.push(getEntry('draggable:drop', location.current.input)),
    ...
    dropTargetForElements({
      ...
      onDrag: ({ location }) =>
        ordered.push(getEntry('dropTarget:drag', location.current.input)),
      onDrop: ({ location }) =>
        ordered.push(getEntry('dropTarget:drop', location.current.input)),
      onDropTargetChange: ({ location }) =>
    ...
    monitorForElements({
      ...
      onDrag: ({ location }) =>
        ordered.push(getEntry('monitor:drag', location.current.input)),
      onDrop: ({ location }) =>
        ordered.push(getEntry('monitor:drop', location.current.input)),
      onDropTargetChange: ({ location }) =>
  ...
  fireEvent.dragEnter(
    document.body,
    getDefaultInput({ clientX: 3, clientY: 4 }),
  );
  expect(ordered).toEqual([

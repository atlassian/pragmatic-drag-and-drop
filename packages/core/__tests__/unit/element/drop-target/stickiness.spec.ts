...
it('should ensure drop target record has `isActiveDueToStickiness` set correctly', () => {
  ...
  const cleanup = combine(
    ...
    dropTargetForElements({
      ...
      onDragStart: args =>
        ordered.push(
          `a:start[isActiveDueToStickiness=${args.self.isActiveDueToStickiness}]`,
        ),
      onDropTargetChange: () => ordered.push('a:update'),
      onDrag: args =>
        ordered.push(
          `a:drag[isActiveDueToStickiness=${args.self.isActiveDueToStickiness}]`,
        ),
      onDragLeave: () => ordered.push('a:leave'),
      onDragEnter: () => ordered.push('a:enter'),
    }),
  ...
  expect(ordered).toEqual([
    'draggable:start',
    'a:start[isActiveDueToStickiness=false]',
  ]);
  ordered.length = 0;

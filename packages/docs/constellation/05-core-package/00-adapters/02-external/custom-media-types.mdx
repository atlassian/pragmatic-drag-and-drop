...
When attaching data to a native drag store (for usage in external `window`s and applications), you specific a "media type" for a string value.
...
You can consume custom media attached from other `window`s or applications leveraging the external adapter.
...
dropTargetForExternal({
  ...
  canDrop: ({ source }) =>
    source.types.includes('application/vnd.trello-card-id'),
  onDrop: ({ source }) => {
    const cardId: string | null = source.getData(

...
- _Potential_ to miss an interaction: let's say a user starts trying to drag before `@atlaskit/pragmatic-drag-and-drop` is loaded, then the user would not be able to perform a drag operation. You can add some instrumentation to detect when these 'misses' occur. In our initial observations we have found the 'readiness' gap to be extremely small.

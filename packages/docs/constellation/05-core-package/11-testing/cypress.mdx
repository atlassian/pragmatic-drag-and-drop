...
You will also need to ensure that `cypress` triggers `DragEvent`s correctly. By default, a `.trigger('dragstart')`, fires an `Event`, not a `DragEvent`. So you will need to set the `eventConstructor` to be `DragEvent`, and pass through a `DataTransfer` (which can be empty).

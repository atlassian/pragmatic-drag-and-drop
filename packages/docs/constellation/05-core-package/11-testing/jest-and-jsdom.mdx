...
Unfortunately, [`jsdom` does not implement `DragEvent`](https://github.com/jsdom/jsdom/issues/2913), and so you will need to setup `DragEvent`s yourself. You can use this [`DragEvent` ponyfill](https://gist.github.com/alexreardon/cbc317ede2212d6db59ba5ff59dcf419), which we plan on exposing in a testing utility package soon.

...
import AccessibilityTriggers from '../../examples/accessibility-triggers';
...
- A draggable entity should almost always have a drag handle icon to clearly show the entity is draggable. See [design guidelines](./design-guidelines).
...
- If no **More button (...)** exists, and movement outcomes are the only possible actions for the entity, then you can convert the drag handle icon into a [drag handle menu button](./optional-packages/react-accessibility) that triggers an inline menu with movement outcomes.
- A draggable entity should _not_ have a **More button (...)** and a [drag handle menu button](./optional-packages/react-accessibility).

<Example Component={AccessibilityTriggers} appearance="showcase-only" />

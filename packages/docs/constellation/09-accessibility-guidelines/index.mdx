...
Please refer to our [design guidelines](./design-guidelines) for guidance about drag handle icons.

</SectionMessage>

- Movement outcomes can be added to a draggable entity by using a **More button (...)**
- Having a **More button (...)** does not remove the need for a drag handle icon. See [design guidelines](./design-guidelines).
- If no **More button (...)** exists on a draggable entity, and movement outcomes are the only possible actions for the entity, then you can convert the drag handle icon into a [drag handle menu button](./optional-packages/react-accessibility) that triggers an inline menu with movement outcomes.
- A draggable entity should only have one button for triggering actions. An entity should _not_ have a **More button (...)** and a [drag handle menu button](./optional-packages/react-accessibility) (an entity should only)

...
The [core package](./core-package) does not enable accessible controls automatically, as there is no one pattern that works well for all situations. Instead, use these guidelines and our [optional packages](./optional-packages) and [Design System components](/components) to create accessible experiences.
...
Provide alternative controls that work well with assistive technologies (for example, a button and a menu) to enable the user trigger outcomes.

### Accessible controls

Use visible and accessible controls (for example, buttons, menus, and forms) to make it easy for people using assistive technologies to trigger the some outcomes as a drag operation.

Our general pattern is to add a button to a draggable entity which can be used to trigger an inline menu (for simple cases), or a modal with a form (for more complex cases). This pattern works well for a lot of cases, but there are other cases where this approach is not appropriate - for those cases please be driven by what would be best for people using assistive technologies.

Your draggable will need have a visible button for triggering outcomes. You can leverage an existing **More actions (...)** button, or your drag handle (which you can make a [drag handle menu](./optional-packages/react-accessibility))

#### Simple cases

When there is only a small set of operations possible for an element, and those operations don't require any input, then we recommend that you add a button to the element that triggers an inline menu containing the possible outcomes.

<Example Component={ListExample} appearance="showcase-only" />

#### Complex cases

If there is a large amount of possible operations, or those operations require further input, we recommend that you add a button to your element that triggers a modal, and inside the modal you can provide a form to filter outcomes and collect input.

<Example Component={TreeExample} appearance="showcase-only" />

<SectionMessage>
  An inline menu can also contain a form, but we currently don't have an example
  of that pattern.
</SectionMessage>

Packages to help you:

- [Drag handle menu component](./optional-packages/react-accessibility/drag-handle-button/examples)
- [inline-menu](/components/inline-menu)
...
For general guidelines on writing meaningful label text, refer to the [accessibility guidelines](/foundations/accessibility/#meaningful-text).
...
- Directional arrow movements require [JAWS screen reader](https://www.freedomscientific.com/products/software/jaws/) users to change screen reader mode to use it
...
To create live region feedback for interactions, use the [live region package](/components/pragmatic-drag-and-drop/packages/live-region).
...
- [Atlassian drag and drop design guidelines](./design-guidelines)
- [Accessibility foundation general guidance](/foundations/accessibility/#meaningful-text)
- [Drag handle menu component](./optional-packages/react-accessibility/drag-handle-button/examples)
- [Live region package](./optional-packages/live-region)

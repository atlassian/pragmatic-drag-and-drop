...
The [core package](./core-package) does not enable accessible controls automatically, as there is no
one pattern that works well for all situations. Instead, use these guidelines and our
[optional packages](./optional-packages) and [Design System components](/components) to create
...
- If no **More button (...)** exists on a draggable entity, and movement outcomes are the only
  ...
  [drag handle menu button](./optional-packages/react-accessibility) that triggers an inline menu
  with movement outcomes.
- A draggable entity should only have one button for triggering actions. An entity should _not_ have
  a **More button (...)** and a [drag handle menu button](./optional-packages/react-accessibility)
...
[live region package](/components/pragmatic-drag-and-drop/packages/live-region).

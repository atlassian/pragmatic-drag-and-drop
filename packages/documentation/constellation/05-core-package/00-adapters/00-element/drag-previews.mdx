...
On iOS `pointerOutsideOfPreview` will center the drag preview under the users pointer.

_Rationale:_

`pointerOutsideOfPreview` uses transparent borders to push the drag preview away from the users
pointer. On iOS these transparent borders will always be black, so we don't enable these borders.

iOS will shift a drag preview so that it is under the center of the users pointer after the drag has
started. So even if we start with the drag with the top left or top right corner of the drag preview
of the users pointer, the position of the drag preview quickly changes so it is under the center of
the users pointer. In order to reduce movement of the drag preview, this function puts the center of
the drag preview under the center

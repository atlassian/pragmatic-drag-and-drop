...
- `dropTargetForExternal`: marking an element as a valid [drop target](../../drop-targets) for
  external entities
- `monitorForExternal`: create a [monitor](../../monitors) to listen for an external drag operation
  events anywhere
- `type`s: all types for this adapter

There are utilities for making working with particular external data types easier:

- [Files](/components/pragmatic-drag-and-drop/files)
- [URLs](/components/pragmatic-drag-and-drop/URLs)
- [Text](/components/pragmatic-drag-and-drop/text)
- [HTML](/components/pragmatic-drag-and-drop/HTML)
- [Custom media](/components/pragmatic-drag-and-drop/custom-media)
...
2. when the user is dropping on a [drop target](../../drop-targets) (including when over a drop
   target due to stickiness)
...
use the optional [`preventUnhandled` utility](../utilities) to help with that.

...
[monitor](/components/pragmatic-drag-and-drop/monitor).
...
([element adapter](/components/pragmatic-drag-and-drop/adapters/element),
[text selection adapter](/components/pragmatic-drag-and-drop/adapters/text-selection)), the drag
finishes when the drag operation finish, which might be while the user is outside of the current
`window`. For drags that start outside of the current `windows`
([external adapter](/components/pragmatic-drag-and-drop/adapters/external)), a drag finishes when

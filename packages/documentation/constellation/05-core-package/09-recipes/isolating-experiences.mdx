...
By default, [drop targets](/components/pragmatic-drag-and-drop/drop-targets) will allow any
`draggable` to be dropped on it, and [monitors](/components/pragmatic-drag-and-drop/monitors) will
listen to _all_ drag operations.
...
[`canDrop()`](/components/pragmatic-drag-and-drop/drop-targets) function is called repeatedly
throughout a drag operation, whereas the monitor
[`canMonitor()`](/components/pragmatic-drag-and-drop/monitors) function is only called once at the
start of a drag (or when a monitor is mounted, if it is mounted during a drag).

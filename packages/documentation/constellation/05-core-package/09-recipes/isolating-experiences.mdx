...
By default, [drop targets](./drop-targets) will allow any `draggable` to be dropped on it, and
[monitors](./monitors) will listen to _all_ drag operations.
...
Something to keep in mind is that the drop target [`canDrop()`](./drop-targets) function is called
repeatedly throughout a drag operation, whereas the monitor [`canMonitor()`](./monitors) function is
only called once at the start of a drag (or when a monitor is mounted, if it is mounted during a
drag).

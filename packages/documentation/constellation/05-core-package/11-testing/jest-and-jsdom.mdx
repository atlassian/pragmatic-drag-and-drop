...
[`jsdom` does not implement `DragEvent`](https://github.com/jsdom/jsdom/issues/2913). We have
created a [`DragEvent` polyfill](/packages/pragmatic-drag-and-drop/optional-packages/unit-testing)
which implements `DragEvent` for `jsdom`.
...
<SectionMessage appearance="information">
	...
	passed as a second parameter of the `fireEvent` drag methods (e.g. `dragStart`, `dragOver`...)
	might be ignored during the test execution. We offer a partial polyfill that you can extend based
	on your needs further below.
</SectionMessage>

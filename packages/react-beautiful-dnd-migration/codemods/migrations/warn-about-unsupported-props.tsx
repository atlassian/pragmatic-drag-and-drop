...
export const unsupportedPropMessages = {
  DragDropContext: {
    nonce:
      'A nonce is no longer needed as all styles are now controlled by React.',
    sensors:
  ...
  Draggable: {
    shouldRespectForcePress:
      'The browser is now responsible for handling this concern.',
  },
  Droppable: {
    isCombineEnabled:
 ...
 */
export function warnAboutUnsupportedProps(
  j: JSCodeshift,
  ...
  forEachRbdElementInFile(j, source, ({ originalName, path }) => {
    ...
    for (const [attributeName, reason] of Object.entries(messages)) {
      const attributeCollection = getJSXAttributesByName(
        j,
        path,
        attributeName,
      );
